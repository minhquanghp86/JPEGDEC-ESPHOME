...
typedef struct jpeg_image_tag
{
    ...
    void *pFramebuffer;
	// compiler to automatically vectorize the C code that is unrolled, the memory arrays must be aligned to 16-byte boundaries
    int16_t sQuantTable[DCTSIZE*4] __attribute__((aligned(16))); // quantization tables
